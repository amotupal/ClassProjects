...
class xor(object):
    ...
    def query_data(self, **kwargs):
        ...
        positive = np.concatenate(
            (positive, np.stack((dim1, dim2), axis=1)), axis=0)
        ...
        negative = np.concatenate(
            (negative, np.stack((dim1, dim2), axis=1)), axis=0)
...
class waldo(object):
    ...
    def _query_sample(self, img):
        ...
        img_sample = imnoise(img_reshaped, mode='gaussian',
                             var=self.var, clip=True)
    ...
    def display_sample(self, sample, title='image'):
        ...
        """
        imshow(sample.reshape(self.sample_height,
                              self.sample_width), window=title)
